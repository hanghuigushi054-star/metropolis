...
import { Grid, BuildingType, TileData, TerrainType } from '../types';
import { GRID_SIZE, BUILDINGS, TERRAIN_COLORS } from '../constants';
...
const ProceduralBuilding = React.memo(({ type, baseColor, x, y, opacity = 1, transparent = false, animateBuild = false }: BuildingMeshProps) => {
  ...
  const isBuilt = useRef(!animateBuild);
  const scaleRef = useRef(animateBuild ? 0 : 1);
  const velocityRef = useRef(0);

  useFrame((state, delta) => {
    if (!isBuilt.current && groupRef.current) {
      // Spring animation values
      const springTension = 200;
      const springFriction = 15;
      
      const dist = 1 - scaleRef.current;
      const force = dist * springTension;
      const acceleration = force - velocityRef.current * springFriction;
      
      velocityRef.current += acceleration * delta;
      scaleRef.current += velocityRef.current * delta;
      
      // Prevent negative scale going crazy, though bounce might dip slightly.
      const currentScale = Math.max(0.001, scaleRef.current);
      groupRef.current.scale.set(currentScale, currentScale, currentScale);
      
      if (Math.abs(dist) < 0.001 && Math.abs(velocityRef.current) < 0.001) {
  ...
  return (
    <group ref={groupRef} rotation={[0, rotation, 0]} position={[0, yOffset, 0]} scale={animateBuild ? [0.001, 0.001, 0.001] : [1,1,1]}>
      {(() => {
        switch (type) {
          ...
          case BuildingType.Office:
            ...
            const officeHeight = 2.0 + hash * 1.5;
            return (
              <>
                <mesh {...commonProps} material={new THREE.MeshStandardMaterial({color: '#1e3a8a', roughness: 0.1, metalness: 0.8})} geometry={boxGeo} position={[0, officeHeight/2, 0]} scale={[0.8, officeHeight, 0.8]} />
                <mesh {...commonProps} material={new THREE.MeshStandardMaterial({color: '#9ca3af'})} geometry={boxGeo} position={[0, officeHeight + 0.1, 0]} scale={[0.85, 0.2, 0.85]} />
              </>
            );
            
          case BuildingType.Highrise:
            // Very tall condo
            const hrHeight = 3.0 + hash * 2.0;
            return (
              <>
                <mesh {...commonProps} material={mainMat} geometry={boxGeo} position={[0, hrHeight/2, 0]} scale={[0.6, hrHeight, 0.6]} />
                <mesh {...commonProps} material={accentMat} geometry={boxGeo} position={[0, hrHeight/2, 0]} scale={[0.65, hrHeight * 0.9, 0.5]} />
                {Array.from({ length: Math.floor(hrHeight * 2) }).map((_, i) => (
                  <WindowBlock key={i} position={[0, 0.5 + i * 0.5, 0]} scale={[0.66, 0.2, 0.66]} />
                ))}
              </>
            );

          case BuildingType.Mall:
            // Wide sprawling building
            return (
              <>
                <mesh {...commonProps} material={mainMat} geometry={boxGeo} position={[0, 0.5, 0]} scale={[0.95, 1.0, 0.95]} />
                <mesh {...commonProps} material={accentMat} geometry={boxGeo} position={[0, 1.1, 0]} scale={[0.8, 0.2, 0.8]} />
                <mesh {...commonProps} material={new THREE.MeshStandardMaterial({color: '#60a5fa', transparent: true, opacity: 0.8})} geometry={boxGeo} position={[0, 1.25, 0]} scale={[0.6, 0.3, 0.6]} />
              </>
            );
          ...
          case BuildingType.PowerPlant:
            ...
            return (
              <>
                ...
                <mesh castShadow receiveShadow material={new THREE.MeshStandardMaterial({color: '#d1d5db'})} geometry={cylinderGeo} position={[-0.2, 1.5, -0.2]} scale={[0.2, 1.0, 0.2]} />
                <mesh castShadow receiveShadow material={new THREE.MeshStandardMaterial({color: '#d1d5db'})} geometry={cylinderGeo} position={[0.2, 1.5, 0.2]} scale={[0.2, 1.0, 0.2]} />
                <SmokeStack position={[-0.2, 1.0, -0.2]} />
                <SmokeStack position={[0.2, 1.0, 0.2]} />
              </>
            );
          ...
          case BuildingType.Stadium:
            ...
            return (
              <>
                <mesh {...commonProps} material={mainMat} geometry={cylinderGeo} position={[0, 0.5, 0]} scale={[0.9, 1.0, 0.9]} />
                <mesh {...commonProps} material={accentMat} geometry={cylinderGeo} position={[0, 1.1, 0]} scale={[0.85, 0.2, 0.85]} />
                <mesh {...commonProps} material={new THREE.MeshStandardMaterial({color: '#4ade80'})} geometry={cylinderGeo} position={[0, 0.9, 0]} scale={[0.7, 0.1, 0.7]} />
              </>
            );
...
const GroundTile = React.memo(({ tile, grid, onHover, onLeave, onClick }: GroundTileProps) => {
  const {x, y, buildingType, terrainType, height, isOwned} = tile;
  const [wx, _, wz] = gridToWorld(x, y);
  ...
  let topY = -0.3 + height; 
  let thickness = 0.5 + height;
  
  if (buildingType !== BuildingType.None && buildingType !== BuildingType.Road) {
    color = '#d1d5db'; // concrete base
  } else if (buildingType === BuildingType.Road) {
    color = '#374151'; // road base
    topY += 0.01;
  } else if (!isOwned && terrainType !== TerrainType.Water) {
    // Dim unowned land
  ...
  return (
    ...
    >
      ...
      <meshStandardMaterial color={color} flatShading roughness={1} opacity={isOwned || terrainType === TerrainType.Water ? 1 : 0.8} transparent={!isOwned} />
      {buildingType === BuildingType.Road && <RoadMarkings x={x} y={y} grid={grid} yOffset={thickness / 2 + 0.001} />}
      {/* Scenery details */}
      {buildingType === BuildingType.None && terrainType === TerrainType.Forest && (
        <group position={[0, thickness/2, 0]}>
          <mesh castShadow receiveShadow material={new THREE.MeshStandardMaterial({ color: '#78350f' })} geometry={new THREE.CylinderGeometry(1, 1, 1, 8)} position={[0, 0.15, 0]} scale={[0.1, 0.3, 0.1]} />
          <mesh castShadow receiveShadow material={new THREE.MeshStandardMaterial({ color: '#14532d', flatShading: true })} geometry={new THREE.ConeGeometry(1, 1, 4)} position={[0, 0.4, 0]} scale={[0.4, 0.5, 0.4]} />
          <mesh castShadow receiveShadow material={new THREE.MeshStandardMaterial({ color: '#166534', flatShading: true })} geometry={new THREE.ConeGeometry(1, 1, 4)} position={[0, 0.65, 0]} scale={[0.3, 0.4, 0.3]} />
        </group>
      )}
      {buildingType === BuildingType.None && terrainType === TerrainType.Stone && (
        <group position={[0, thickness/2, 0]}>
           <mesh castShadow receiveShadow material={new THREE.MeshStandardMaterial({ color: '#6b7280', flatShading: true })} geometry={new THREE.SphereGeometry(1, 4, 4)} position={[0.2, 0.1, -0.2]} scale={0.15} rotation={[Math.PI/6, Math.PI/4, 0]} />
           <mesh castShadow receiveShadow material={new THREE.MeshStandardMaterial({ color: '#9ca3af', flatShading: true })} geometry={new THREE.SphereGeometry(1, 4, 4)} position={[-0.1, 0.1, 0.1]} scale={0.2} rotation={[0, Math.PI/3, 0]} />
        </group>
      )}

      {!isOwned && terrainType !== TerrainType.Water && (
...
const Cursor = ({ x, y, color, height = 0, isUnowned = false, cost = 0, canAfford = false, isWater = false }: { x: number, y: number, color: string, height?: number, isUnowned?: boolean, cost?: number, canAfford?: boolean, isWater?: boolean }) => {
  const [wx, _, wz] = gridToWorld(x, y);
  return (
    <mesh position={[wx, -0.25 + height, wz]} rotation={[-Math.PI / 2, 0, 0]} raycast={() => null}>
      <planeGeometry args={[1, 1]} />
      {isUnowned && !isWater ? (
          <meshBasicMaterial color={canAfford ? "#eab308" : "#ef4444"} transparent opacity={0.6} side={THREE.DoubleSide} depthTest={false} />
      ) : (
...
const IsoMap: React.FC<IsoMapProps> = ({ grid, onTileClick, hoveredTool, population, level, money, gameStarted = true }) => {
  ...
  // Preview Logic
  const hoveredGridTile = hoveredTile ? grid[hoveredTile.y][hoveredTile.x] : null;
  const isUnowned = hoveredGridTile ? !hoveredGridTile.isOwned : false;
  const isWater = hoveredGridTile ? hoveredGridTile.terrainType === TerrainType.Water : false;
  const showPreview = hoveredGridTile && hoveredGridTile.buildingType === BuildingType.None && hoveredTool !== BuildingType.None && !isUnowned;
  const previewColor = showPreview ? BUILDINGS[hoveredTool].color : 'white';
  const isBulldoze = hoveredTool === BuildingType.None;
  const landCost = 200 + (level * 100);
  const canAffordLand = money >= landCost;
        ...
        <group>
          {grid.map((row, y) =>
            row.map((tile, x) => {
              ...
              <React.Fragment key={`${x}-${y}`}>
                ...
                <group position={[wx, tile.height, wz]} raycast={() => null}>
          ...
          <group raycast={() => null}>
            ...
            {showPreview && hoveredTile && (
              <group position={[previewPos[0], grid[hoveredTile.y][hoveredTile.x].height, previewPos[2]]}>
            ...
            {hoveredGridTile && (
              <Cursor 
                x={hoveredGridTile.x} 
                y={hoveredGridTile.y} 
                height={hoveredGridTile.height}
                isUnowned={isUnowned}
                cost={landCost}
                canAfford={canAffordLand}
                isWater={isWater}
